...
int mm_is_Axid(MM_typecode matcode);		/* too complex for a macro */
        ...
        double **Ax, MM_typecode *matcode)
{
    ...
    if (!(mm_is_Axid(*matcode) && mm_is_sparse(*matcode) &&
    ...
    *Ax = NULL;
    ...
    if (mm_is_complex(*matcode))
    {
        *Ax = (double *) malloc(*nz * 2 * sizeof(double));
        ret_code = mm_read_mtx_crd_data(f, *M, *N, *nz, *I, *J, *Ax,
                *matcode);
        ...
        double Ax[], MM_typecode matcode)
{
    ...
    {
        for (i=0; i<nz; i++)
            if (fscanf(f, "%d %d %lg %lg", &I[i], &J[i], &Ax[2*i], &Ax[2*i+1])
